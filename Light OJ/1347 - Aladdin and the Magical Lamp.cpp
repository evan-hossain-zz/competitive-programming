...
void printSA()
{
    for(int i=0;i<SZ(text);i++) printf("%d %s\n", i, text.substr(SA[i]).c_str());
    puts("");
...
int get_pos(int indx)
{
    for(int i = 0; i < 3; i++)
    {
        if(indx >= end_points[i].x && indx < end_points[i].y)
            return i;
    }
    return -1;
}
...
bool ok(int val)
{
    ...
    for(i = 1; i < n; i++)
    {
        if(lcp[i] >= val)
        {
            ...
            if(tem == -1) continue;
            a = b = c = false;
            tem == 0? a = true: tem == 1? b = true: tem == 2? c = true : c = false;
            tem = get_pos(SA[i - 1]);
            tem == 0? a = true: tem == 1? b = true: tem == 2? c = true : c = false;
            r = i + 1;
            while(r < n && lcp[r] >= val && !(a && b && c))
            {
                tem = get_pos(SA[r]);
                tem == 0? a = true: tem == 1? b = true: tem == 2? c = true : c = false;
                r++;
...
//    in;
//    out;
    int test, kase = 1, low, high, mid, res;
    scanf("%d", &test);
    while(test--)
    {
        scanf(" %s", str);
        text = str;
        end_points[0].x = 0;
        end_points[0].y = SZ(text);
        scanf(" %s", str);
        text += "#";
        end_points[1].x = SZ(text);
        text += str;
        end_points[1].y = SZ(text);
        scanf(" %s", str);
        text += "#";
        end_points[2].x = SZ(text);
        text += str;
        end_points[2].y = SZ(text);
        buildSA(SZ(text));
        buildLCP(SZ(text));
//        printSA();
        low = 0, high = SZ(text);
