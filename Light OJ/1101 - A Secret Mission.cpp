...
void process(int n)
{
    ...
    for(lev = 1; lev <= h; lev++)
    {
        for(i = 1; i <= n; i++)
        {
            if(pwr[i][lev - 1] != -1)
            {
                arr[i][lev] = min(arr[i][lev - 1], arr[pwr[i][lev - 1]][lev - 1]);
...
int query(int high, int low)
{
    ...
    for(i = h; i >= 0; i--)
    {
        ...
        {
            ret = min(ret, arr[low][i]);
            ret = min(ret, arr[high][i]);
            low = pwr[low][i];
            high = pwr[high][i];
        }
    }
    ret = min(ret, arr[low][0]);
    ret = min(ret, arr[high][0]);
    return ret;
}
