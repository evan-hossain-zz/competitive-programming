...
int match[MAX + MAX], starting[MAX], visited[MAX];
...
int get()
{
    ...
    for(i = 0; i < SZ(text); i++)
    {
        add = min(starting[SA[i]] + SZ(B) - 1, SZ(text)) - SA[i] - lcp[i];
        if(add > 0)
            ret += add;
    }
    return ret;
...
//        out;
    #endif
    int test, kase = 1, i, cur, last, now = 0;
    scanf("%d", &test);
    while(test--)
    {
        scanf(" %s", str);
        ...
        now++;
        compute_match_array(B + "#" + text);
        for(i = 0; i < SZ(text); i++)
        {
            cur = i + SZ(B) + 1;
            if(match[cur] == SZ(B))
                starting[i - SZ(B) + 1] = i - SZ(B) + 1, visited[i - SZ(B) + 1] = now;
        }
        last = SZ(text);
        for(i = SZ(text) - 1; i >= 0; i--)
        {
            if(visited[i] == now)
                last = starting[i];
            else
