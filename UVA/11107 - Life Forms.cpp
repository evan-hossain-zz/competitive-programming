...
void buildLCP(int n)
{
    ...
    for(int i =0 ; i< n ; i++)
    {
        ...
        int j = SA[k-1];
        while(text[i+len]==text[j+len] && text[i + len] != '#') len++;
        lcp[k] = len;
        if(len) len--;
    }
    return;
...
void printSA()
{
    for(int i=0;i<SZ(text);i++) printf("%d %d %s %d\n", i, SA[i], text.substr(SA[i]).c_str(), lcp[i]);
    puts("");
...
bool get(int k, bool print)
{
    int l = 0, r, n = SZ(text), tem;
    while(l < n)
    {
        if(lcp[l] < k)
        {
        ...
        tem = 0;
        if(pos[SA[l]] != -1)
            valid[pos[SA[l]]] = 1, tem++;
        if(pos[SA[l - 1]] != -1 && !valid[pos[SA[l - 1]]])
            valid[pos[SA[l - 1]]] = 1, tem++;
        r = l + 1;
        while(r < n && lcp[r] >= k)
        {
            if(pos[SA[r]] != -1 && !valid[pos[SA[r]]])
                valid[pos[SA[r]]] = 1, tem++;
            r++;
        }
        if(tem >= total / 2  + 1)
        {
