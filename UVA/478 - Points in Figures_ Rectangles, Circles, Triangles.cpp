...
int main()
{
    ...
    while(cin >> ch && ch != '*')
    {
        ...
        {
            fix_triangle.clear();
            cin >> ttem.a.x >> ttem.a.y >> ttem.c.x >> ttem.c.y >> ttem.b.x >> ttem.b.y;
            ttem.fig = fig++;
            tri.pb(ttem);
        }
    }
    while(cin >> p.x >> p.y && !(p.x == 9999.9 && p.y == 9999.9))
    {
        ...
        {
            point centre;
            centre.x = cir[i].x, centre.y = cir[i].y;
            if(getdist(centre, p) < cir[i].r)
                ans.pb(cir[i].fig);
        }
        for(i = 0; i < tri.size(); i++)
        {
            if((check(tri[i].a, tri[i].b, p) == check(tri[i].b, tri[i].c, p)) && (check(tri[i].b, tri[i].c, p) == check(tri[i].c, tri[i].a, p)))
                ans.pb(tri[i].fig);
        }
        if(ans.empty())
        {
