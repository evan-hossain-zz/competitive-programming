...
public:
	int minTime(vector <string> arr) {
		vector <int> smask(SZ(arr), 0);
		vector <int> dp(1 << 20, 1<<28);
		dp[0] = 0;
		int ret = 0;
		...
		for(int i = 0; i < (1 << 20); i++)
        {
            ...
            {
                int newMask = i | smask[j];
                int cost = no_of_ones(i ^ newMask);
                dp[newMask] = min(dp[newMask], dp[i] + sqr(cost));
            }
        }
        return dp[ret];
	}
