...
int query(node *a, node *b, int l, int r, int k)
{
    if(l == r)
        return l;
    int tot = a->left->val - b->left->val;
    int m = (l+r) >> 1;
    if(tot >= k)
        return query(a->left, b->left, l, m, k);
    return query(a->right, b->right, m+1, r, k-tot);
}
...
//        out;
    ...
    while(m--)
    {
        cin >> u >> v >> k;
        u--, v--;
        res = query(rootOf[v], par[u][0] == -1? root: rootOf[par[u][0]], 0, SZ(mpp)-1, k);
        cout << orgVal[res] << "\n";
    }
    return 0;
}
